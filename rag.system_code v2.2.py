...
from langchain_chroma import Chroma  # Swapped out FAISS for Chroma
...
# Path where Chroma will store data on your disk locally
CHROMA_PERSIST_DIR = os.path.join(SCRIPT_DIR, "chroma_db_storage")

# Clean up previous local database instances to ensure an evaluation blank-slate run
if os.path.exists(CHROMA_PERSIST_DIR):
    print(f"Cleaning old Chroma instance at {CHROMA_PERSIST_DIR}...")
...
# ==========================================
# STEP 4: VECTOR DATABASE STORE (CHROMA DB)
# ==========================================
print("Populating local persistent Chroma DB structure index...")
vector_db = Chroma.from_documents(
    documents=chunks,
    embedding=embeddings,
    persist_directory=CHROMA_PERSIST_DIR
)
print("Chroma DB successfully initialized and persisted locally.")

# ==========================================
# STEP 5: AUTOMATED EVALUATION LOOP
# ==========================================
print("\n" + "="*60)
print(" RUNNING EVALUATION SUITE: VERSION 2.0 (CHROMA DB) ")
print("="*60 + "\n")

for item in EVAL_QUESTIONS:
    print(f"Executing: {item['id']} ({item['category']})")
    
    # Retrieve top 3 relevant chunks via Cosine Similarity from Chroma
    retrieved_docs = vector_db.similarity_search(item["text"], k=10)
    
    # Bundle retrieved context chunks cleanly
    context = "\n\n".join([doc.page_content for doc in retrieved_docs])
    
    # Strict context constraint instruction
    prompt = f"""
